    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex flex-col gap-4">
                <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        <button
                            ...
                            className={`flex items-center gap-2 px-3 py-2.5 border rounded-lg text-sm transition-colors w-full sm:w-auto justify-center ${showFilters ? 'bg-primary/10 border-primary/50 text-primary' : 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10'}`}
                        >
                            <Filter size={14} />
                            <span>Filter</span>
                        </button>
                ...
                {/* Expanded Filters */}
                {showFilters && (
                    <div className="flex flex-wrap gap-2 p-4 bg-white/5 rounded-lg border border-white/10 animate-in fade-in slide-in-from-top-2">
                        <span className="text-sm text-gray-400 mr-2 flex items-center">Status:</span>
                        {['ALL', 'COMPLETED', 'RUNNING', 'ERROR'].map((status) => (
                            <button
                                key={status}
                                onClick={() => setStatusFilter(status)}
                                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors border ${statusFilter === status
